...
import { LayoutDashboard, Users, LogOut, Megaphone, Calendar, FileSpreadsheet, Printer } from 'lucide-react';
...
function MenuBtn({ to, icon, label, end = false }) {
  return (
    <NavLink to={to} end={end} className={({ isActive }) => `w-full flex items-center gap-4 px-5 py-4 rounded-2xl transition-all font-bold text-sm ${isActive ? 'bg-sora-blue shadow-lg shadow-sora-blue/20 text-white' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}>
      {icon} {label}
    </NavLink>
  );
}
    ...
    <div className="flex h-screen bg-sora-bg font-sans overflow-hidden text-left print:hidden">
      <aside className="w-72 bg-sora-navy text-white flex flex-col shadow-2xl z-20">
        ...
        <nav className="flex-1 p-6 space-y-2 overflow-y-auto custom-scrollbar">
          <MenuBtn to="/admin" end icon={<LayoutDashboard size={20}/>} label="Dashboard" />
          <MenuBtn to="/admin/siswa" icon={<Users size={20}/>} label="Data Siswa" />
          <MenuBtn to="/admin/laporan" icon={<FileSpreadsheet size={20}/>} label="Laporan Keuangan" />
          <MenuBtn to="/admin/broadcast" icon={<Megaphone size={20}/>} label="Broadcast" />
          <MenuBtn to="/admin/kalender" icon={<Calendar size={20}/>} label="Kalender Akademik" />
        </nav>
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative">
        <header className="bg-white px-10 py-6 flex justify-between items-center border-b shadow-sm z-10">
          <div><h2 className="text-2xl font-black text-sora-navy uppercase tracking-tight">{getPageTitle()}</h2></div>
